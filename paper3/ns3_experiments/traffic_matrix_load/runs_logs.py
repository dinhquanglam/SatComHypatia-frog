...
dossiers=sorted([runsdoss for doss in os.listdir("runs") if os.path.isdir(runsdoss:="runs/"+doss) and '240s' in doss])
for doss in dossiers:
	...
	if fics:
		...
		axes[0].plot(t_cwnds,data_cwnds)
		...
		axes[2].plot(t_prgs,data_prgs)
		#plt.savefig(doss+"_logs.png")
plt.show()
